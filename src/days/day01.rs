...
fn times_at_zero(start: i16, rotations: &[i16]) -> (i16, i16) {
    ...
    for rotation in rotations {
        ...
        if next <= 0 && dial_position != 0 || next > 99 {
            turning_zeroes += 1;
        }

        // Update current dial position with normalised next value.
        dial_position = match next {
            ..0 => next + 100,
            0..=99 => next,
            100.. => next - 100,
        };
